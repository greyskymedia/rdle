...
function Row1() {
  ...
  const [title, titleClass] = practice
    ? id < PRACTICE_MODE_MIN_ID
      ? [`Historical Rdle #${id}`, "historical"]
      : [`Practice Rdle`, "practice"]
    : [`Daily Rdle #${id}`, null];
  ...
  const handleNewClick = () => {
    ...
    const res = window.confirm(
      "Are you sure you want to start a new Practice Rdle?\n" +
  ...
  const handleHistClick = () => {
    ...
    const res = window.prompt(
      "Play a Historical Rdle!\n" +
        "Enter a past Daily Rdle # to play:"
    );
    if (res === null) return;
    ...
    if (isNaN(num)) {
      ...
      alert(
        `Starting Practice Rdle with seed ${num}\n` +
          `(Note: you can set a Practice Rdle seed ≥100000)`
      );
      dispatch(startGame({ id: num, practice: true }));
    } else {
      alert("Please enter a past Daily Rdle #");
    }
  };
